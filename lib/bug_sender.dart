...
class _BugSenderPageState extends State<BugSenderPage>
  ...
  Future<void> _fetchSenders() async {
    ...
    try {
      final res = await http.get(
        Uri.parse("http://server.lynzzofficial.com:2014/mySender?key=${widget.sessionKey}"),
      );
  ...
  Future<void> _addSender(String number, bool isGlobal) async {
    ...
    try {
      final res = await http.get(
        Uri.parse(
          "http://server.lynzzofficial.com:2014/getPairing?key=${widget.sessionKey}&number=$number&global=${isGlobal ? 1 : 0}",
        ),
      );
      final data = jsonDecode(res.body);
      if (res.statusCode == 200 && data["valid"] == true) {
  ...
  Future<void> _deleteSender(String id, bool isGlobal) async {
    ...
    try {
      final res = await http.delete(
        Uri.parse(
          "http://server.lynzzofficial.com:2014/deleteSender?key=${widget.sessionKey}&id=$id&scope=${isGlobal ? 'global' : 'private'}",
        ),
      );
      final data = jsonDecode(res.body);
      if (res.statusCode == 200 && data["valid"] == true) {
